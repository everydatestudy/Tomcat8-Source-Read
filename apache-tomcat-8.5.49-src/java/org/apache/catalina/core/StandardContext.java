...
public class StandardContext extends ContainerBase implements Context, NotificationEmitter {
	...
	/**
	 * Allow multipart/form-data requests to be parsed even when the target servlet
	 * doesn't specify @MultipartConfig or have a &lt;multipart-config&gt; element.
	 */
	protected boolean allowCasualMultipartParsing = false;

	/**
	 * Control whether remaining request data will be read (swallowed) even if the
	 * request violates a data size constraint.
	 */
	 ...
	 * in the order they were encountered in the resulting merged web.xml file.
	 */
	...
	/**
	 * The set of application listeners that are required to have limited access to
	 * ServletContext methods. See Servlet 3.1 section 4.4.
	 */
	private final Set<Object> noPluggabilityListeners = new HashSet<>();

	/**
	 * The list of instantiated application event listener objects. Note that SCIs
	 * and other code may use the pluggability APIs to add listener instances
	 * directly to this list before the application starts.
	 */
	private List<Object> applicationEventListenersList = new CopyOnWriteArrayList<>();

	/**
	 * The set of instantiated application lifecycle listener objects. Note that
	 * SCIs and other code may use the pluggability APIs to add listener instances
	 * directly to this list before the application starts.
	 */
	...
	/**
	 * The wrapped version of the associated ServletContext that is presented to
	 * listeners that are required to have limited access to ServletContext methods.
	 * See Servlet 3.1 section 4.4.
	 */
	...
	/**
	 * Should we allow the <code>ServletContext.getContext()</code> method to access
	...
	/**
	 * The set of filter configurations (and associated filter instances) we have
	 * initialized, keyed by filter name.
	 */
	 ...
	 * defined in the deployment descriptor with additional mappings added via the
	 * {@link ServletContext} possibly both before and after those defined in the
	 ...
	 * descriptor version we are currently parsing. This is used to support relaxed
	 * validation rules when processing version 2.2 web.xml files.
	 */
	...
	/**
	 * Should the next call to <code>addWelcomeFile()</code> cause replacement of
	 * any existing welcome files? This will be set before processing the web
	 * application's deployment descriptor, so that application specified choices
	 * <strong>replace</strong>, rather than append to, those defined in the global
	 * descriptor.
	 */
	private boolean replaceWelcomeFiles = false;

	/**
	 * The security role mappings for this application, keyed by role name (as used
	...
	/**
	 * Set flag to true to cause the system.out and system.err to be redirected to
	 * the logger when executing a servlet.
	 */
	...
	/**
	 * The set of classnames of ContainerListeners that will be added to each newly
	 * created Wrapper by <code>createWrapper()</code>.
	 */
	...
	/**
	 * The pathname to the work directory for this context (relative to the server's
	...
	/**
	 * The domain to use for session cookies. <code>null</code> indicates that the
	 * domain is controlled by the application.
	 */
	 ...
	 * particularly IE, don't send a session cookie for context /foo with requests
	 * intended for context /foobar.
	 */
	private boolean sessionCookiePathUsesTrailingSlash = false;

	/**
	 * The Jar scanner to use to search for Jars that might contain configuration
	 * information such as TLDs or web-fragment.xml files.
	 */
	...
	/**
	 * Should Tomcat attempt to terminate threads that have been started by the web
	 * application? Stopping threads is performed via the deprecated (for good
	 * reason) <code>Thread.stop()</code> method and is likely to result in
	 * instability. As such, enabling this should be viewed as an option of last
	 * resort in a development environment and is not recommended in a production
	 * environment. If not specified, the default value of <code>false</code> will
	 * be used.
	 */
	private boolean clearReferencesStopThreads = false;

	/**
	 * Should Tomcat attempt to terminate any {@link java.util.TimerThread}s that
	 * have been started by the web application? If not specified, the default value
	 ...
	 * keep-alive timer thread will stop on its own once the keep-alives all expire
	 * however, on a busy system that might not happen for some time.
	 */
	private boolean clearReferencesHttpClientKeepAliveThread = true;

	/**
	 * Should Tomcat renew the threads of the thread pool when the application is
	 * stopped to avoid memory leaks because of uncleaned ThreadLocal variables.
	 * This also requires that the threadRenewalDelay property of the
	...
	public void setResponseCharacterEncoding(String responseEncoding) {
		/*
		 * This ensures that the context response encoding is represented by a unique
		 * String object. This enables the Default Servlet to differentiate between a
		 * Response using this default encoding and one that has been explicitly
		 * configured.
		 */
	...
	/**
	 * Set to <code>true</code> to allow requests mapped to servlets that do not
	 * explicitly declare @MultipartConfig or have &lt;multipart-config&gt;
	 * specified in web.xml to parse multipart/form-data requests.
	 *
	 * @param allowCasualMultipartParsing <code>true</code> to allow such casual
	 *                                    parsing, <code>false</code> otherwise.
	 */
	@Override
	...
	/**
	 * Returns <code>true</code> if requests mapped to servlets without "multipart
	 * config" to parse multipart/form-data requests anyway.
	 *
	 * @return <code>true</code> if requests mapped to servlets without "multipart
	 *         config" to parse multipart/form-data requests, <code>false</code>
	 *         otherwise.
	 */
	@Override
	...
	/**
	 * Set to <code>false</code> to disable request data swallowing after an upload
	 * was aborted due to size constraints.
	 *
	...
	/**
	 * Returns <code>true</code> if remaining request data will be read (swallowed)
	 * even the request violates a data size constraint.
	 *
	 ...
	 * ClassLoader.
	 *
	 * @return <code>true</code> if classloading delegates to the parent classloader
	 *         first
	 */
	 ...
	 * {@inheritDoc}
	 *
	 * Note that this implementation is not thread safe. If two threads call this
	 * method concurrently, the result may be either set of listeners or a the union
	 * of both.
	 */
	@Override
	...
	/**
	 * Store the set of initialized application lifecycle listener objects, in the
	 * order they were specified in the web application deployment descriptor, for
	 * this application.
	 *
	...
	/**
	 * Add a listener to the end of the list of initialized application lifecycle
	 * listeners.
	 *
	 * @param listener The listener to add
	 */
	 ...
	 * false by startup listeners that detect a fatal configuration error to avoid
	 * the application from being made available.
	 *
	...
	/**
	 * Gets the name to use for session cookies. Overrides any setting that may be
	...
	/**
	 * Sets the name to use for session cookies. Overrides any setting that may be
	 * specified by the application.
	 *
	...
	/**
	 * Gets the domain to use for session cookies. Overrides any setting that may be
	 * specified by the application.
	 *
	...
	/**
	 * Sets the domain to use for session cookies. Overrides any setting that may be
	 * specified by the application.
	 *
	...
	/**
	 * Gets the path to use for session cookies. Overrides any setting that may be
	 * specified by the application.
	 *
	...
	/**
	 * Sets the path to use for session cookies. Overrides any setting that may be
	 * specified by the application.
	 *
	 ...
	 * absolute, it'll be made relative to the engine's base dir ( which defaults to
	...
	/**
	 * Set the location of the default web xml that will be used. If not absolute,
	 * it'll be made relative to the engine's base dir ( which defaults to
	 * catalina.base system property ).
	 *
	...
	/**
	 * Set the public identifier of the deployment descriptor DTD that is currently
	 * being parsed.
	 *
	 ...
	 *         pathname, a relative pathname, or a URL. Is only set as deployment
	 *         has change docRoot!
	 */
	...
	/**
	 * @return the parent class loader (if any) for this web application. This call
	 *         is meaningful only <strong>after</strong> a Loader has been
	 *         configured.
	 */
	@Override
	...
	/**
	 * Set the value of the swallowOutput flag. If set to true, the system.out and
	 * system.err will be redirected to the logger during a servlet execution.
	 *
	...
	/**
	 * Set the value of the unloadDelay flag, which represents the amount of ms that
	 * the container will wait when unloading servlets. Setting this to a small
	 * value may cause more requests to fail to complete when stopping a web
	 * application.
	 *
	 ...
	 *         included in a WAR will be copied to the host configuration base
	...
	/**
	 * @return the Java class name of the Wrapper implementation used for servlets
	 ...
	 *
	 * @throws IllegalArgumentException if the specified wrapper class cannot be
	 *                                  found or is not a subclass of
	 *                                  StandardWrapper
	 */
	@Override
	...
	/**
	 * 1.Pre。如: context.xml下的配置<PreResource>xxxxx</PreResource> 2
	 * Main资源。WEB-INFO/的资源。包括classes & lib. 3.Jar资源。 <JarResource>xxx.</JarResource>
	...
	/**
	 * @return whether or not an attempt to modify the JNDI context will trigger an
	 *         exception or if the request will be ignored.
	 */
	...
	/**
	 * Controls whether or not an attempt to modify the JNDI context will trigger an
	 * exception or if the request will be ignored.
	 *
	...
	public void addChild(Container child) {
		...
		if (isJspServlet && oldJspServlet != null) {
			/*
			 * The webapp-specific JspServlet inherits all the mappings specified in the
			 * global web.xml, and may add additional ones.
			 */
	...
	/**
	 * Add a filter mapping to this Context at the end of the current set of filter
	 * mappings.
	 *
	 * @param filterMap The filter mapping to be added
	 *
	 * @exception IllegalArgumentException if the specified filter name does not
	 *                                     match an existing filter definition, or
	...
	private void validateFilterMap(FilterMap filterMap) {
		...
		/*
		 * if ((servletNames.length != 0) && (urlPatterns.length != 0)) throw new
		 * IllegalArgumentException (sm.getString("standardContext.filterMap.either"));
		 */
	 ...
	 *
	 * @exception IllegalArgumentException if the name or value is missing, or if
	 *                                     this context initialization parameter has
	...
	/**
	 * Add the classname of a LifecycleListener to be added to each Wrapper appended
	 * to this Context.
	 *
	...
	/**
	 * Add the classname of a ContainerListener to be added to each Wrapper appended
	 * to this Context.
	 *
	...
	/**
	 * Return the security constraints for this web application. If there are none,
	...
	/**
	 * Return the filter definition for the specified filter name, if any; otherwise
	...
	/**
	 * @return the message destination with the specified name, if any; otherwise,
	 *         return <code>null</code>.
	 *
	...
	/**
	 * @return the set of defined message destinations for this web application. If
	...
	/**
	 * @return the value for the specified context initialization parameter name, if
	...
	/**
	 * @return the names of all defined context initialization parameters for this
	 *         Context. If no parameters are defined, a zero-length array is
	 *         returned.
	 */
	@Override
	 ...
	 * corresponding role name (as defined by the underlying Realm) if there is one.
	 * Otherwise, return the specified role unchanged.
	 *
	...
	/**
	 * @return <code>true</code> if the specified security role is defined for this
	 *         application; otherwise return <code>false</code>.
	 *
	...
	/**
	 * @return the servlet name mapped by the specified pattern (if any); otherwise
	 *         return <code>null</code>.
	 *
	...
	/**
	 * @return the patterns of all defined servlet mappings for this Context. If no
	 *         mappings are defined, a zero-length array is returned.
	 */
	@Override
	...
	/**
	 * @return <code>true</code> if the specified welcome file is defined for this
	 *         Context; otherwise return <code>false</code>.
	 *
	...
	/**
	 * @return the set of watched resources for this Context. If none are defined, a
	 ...
	 * required by changes to classes in the underlying repositories of our class
	 * loader and changes to the web.xml file. It does not handle changes to any
	 * context.xml file. If the context.xml has changed, you should stop this
	 * Context and create (and start) a new Context instance instead. Note that
	 * there is additional code in <code>CoyoteAdapter#postParseRequest()</code> to
	 * handle mapping requests to paused Contexts.
	 *
	 * @exception IllegalStateException if the <code>reloadable</code> property is
	...
	/**
	 * Remove the specified application listener class from the set of listeners for
	 * this application.
	 *
	...
	/**
	 * Remove the MIME mapping for the specified extension, if it exists; otherwise,
	 * no action is taken.
	 *
	...
	/**
	 * Remove the context initialization parameter with the specified name, if it
	...
	/**
	 * Remove the specified watched resource name from the list associated with this
	 * Context.
	 *
	...
	/**
	 * Remove a class name from the set of LifecycleListener classes that will be
	...
	/**
	 * Remove a class name from the set of ContainerListener classes that will be
	 * added to newly created Wrappers.
	 *
	...
	/**
	 * Return the real path for a given virtual path, if possible; otherwise return
	 * <code>null</code>.
	 *
	...
	private static final class ContextFilterMaps {
		 ...
		 * before the mappings in the deployment descriptor but must be inserted in the
		 * order the {@link ServletContext} methods are called. This isn't an issue for
		 * the mappings added after the deployment descriptor - they are just added to
		 * the end - but correctly the adding mappings before the deployment descriptor
		 * mappings requires knowing where the last 'before' mapping was added.
		 */
		...
		/**
		 * Add a filter mapping before the mappings defined in the deployment descriptor
		 * but after any other mappings added via this method.
		 *
		 * @param filterMap The filter mapping to be added
		 */
	 ...
	 * 
	 * @return <code>true</code> if all filter finalization completed successfully,
	...
	/**
	 * Find and return the initialized <code>FilterConfig</code> for the specified
	 * filter name, if any; otherwise return <code>null</code>.
	 *
	 ...
	 * Context.
	 * 
	 * @return <code>true</code> if all listeners wre initialized successfully, or
	 *         <code>false</code> otherwise.
	 */
	public boolean listenerStart() {
		...
		for (int i = 0; i < results.length; i++) {
			if ((results[i] instanceof ServletContextAttributeListener)
					...
					|| (results[i] instanceof ServletRequestListener) || (results[i] instanceof HttpSessionIdListener)
	 ...
	 * 
	 * @return <code>true</code> if all events were sent successfully, or
	 *         <code>false</code> otherwise.
	 */
	...
	/**
	 * 加载和初始化web.xml中所有标记着"load on startup"的servlets Load and initialize all
	 * servlets marked "load on startup" in the web application deployment
	 * descriptor.
	 *
	 * @param children Array of wrappers for all currently defined servlets
	...
	/**
	 * 启动这个组件并实现基本的需求(很重要的地方) Start this component and implement the requirements of
	...
	protected synchronized void startInternal() throws LifecycleException {
		...
		// 先将Context 的available 和 configured 属性都设置为false
		// 表示当前的StandardContext实例不可以用 且 没有配置成功
		setConfigured(false);
		// start方法中 正确执行的状态值 由于下面的代码可能会出现某些意外情况导致 启动失败 ok为 下面代码是否可以正确执行的标志
		boolean ok = true;
		...
		try {
			if (ok) {
				...
				/**
				 * 发布config_start事件。 这里面做了很多很多的事情。 包括加载Web.xml中的内容到Context上下文中。
				 * 以及初始化Spring的ContextLoaderListener监听器。 ContextLoaderListener实现
				 * ServletContextListener 接口，继承 ContextLoader 加载器。 把Tomcat与Spring 连接到了一起。
				 */
			...
			/**
			 * 加载并初始化所有的多个Listener。 包括Web.xml内配置的Spring的ContextLoaderListener 监听器。
			 * ContextLoaderListener 通过实现 ServletContextListener 接口， 继承 ContextLoader 加载器。 把
			 * Tomcat 与 spring 连接到了一起。
			 */
			if (ok) {
	 ...
	 * deployment descriptor with the application parameters described in the server
	 * configuration, respecting the <code>override</code> property of the
	 * application parameters appropriately.
	 */
	 ...
	 *
	 * The problem is that undoing all the config in start() and restoring a 'fresh'
	 * state is impossible. After stop()/destroy()/init()/start() we should have the
	 * same state as if a fresh start was done - i.e read modified web.xml, etc.
	 * This can only be done by completely removing the context object and remapping
	 * a new one, or by cleaning up everything.
	 *
	...
	/**
	 * Adjust the URL pattern to begin with a leading slash, if appropriate (i.e. we
	 * are running a servlet 2.2 application). Otherwise, return the specified URL
	 * pattern unchanged.
	 *
	 ...
	 * startup, shutdown and reloading of the context.
	 *
	...
	/**
	 * Check for unusual but valid <code>&lt;url-pattern&gt;</code>s. See Bugzilla
	 * 34805, 43079 & 43080
	 */
	 ...
	 * 
	 * @see javax.management.NotificationEmitter#removeNotificationListener(javax.
	...
	private static class NoPluggabilityServletContext implements org.apache.catalina.servlet4preview.ServletContext {
