 ...
 * Requirements:
 * <ul>
 * <li>all tomcat classes and possibly servlets are in the classpath. (for
 * example all is in one big jar, or in eclipse CP, or in any other
 * combination)</li>
 *
 * <li>we need one temporary directory for work files</li>
 *
 * <li>no config file is required. This class provides methods to use if you
 * have a webapp with a web.xml file, but it is optional - you can use your own
 * servlets.</li>
 * </ul>
 *
 * <p>
 ...
 *
 * @see <a href=
 *      "https://svn.apache.org/repos/asf/tomcat/trunk/test/org/apache/catalina/startup/TestTomcat.java">TestTomcat</a>
...
public class Tomcat {
	 ...
	 * and any WEB-INF/web.xml and META-INF/context.xml packaged with the
	 ...
	 * {@link #addWebapp(String, String)} with the newly copied WAR. The WAR will
	 * <b>NOT</b> be removed from the Host's appBase when the Tomcat instance stops.
	 * Note that {@link ExpandWar} provides utility methods that may be used to
	 * delete the WAR and/or expanded directory if required.
	 *
	 * @param contextPath The context mapping to use, "" for root context.
	 ...
	 * @return The deployed Context
	 *
	 * @throws IOException If an I/O error occurs while copying the WAR file from
	 *                     the specified URL to the appBase
	 */
	...
	/**
	 * Add a context - programmatic mode, no default web.xml used. This means that
	 * there is no JSP support (no JSP servlet), no default servlet and no web
	 * socket support unless explicitly enabled via the programmatic interface.
	 * There is also no {@link javax.servlet.ServletContainerInitializer} processing
	 * and no annotation processing. If a
	 * {@link javax.servlet.ServletContainerInitializer} is added programmatically,
	 * there will still be no scanning for
	 ...
	 *
	 * <p>
	 * Note: If you reload the Context, all your configuration will be lost. If you
	 * need reload support, consider using a LifecycleListener to provide your
	 * configuration.
	 *
	 * <p>
	 * TODO: add the rest
	 ...
	 * param - this one can be used if the servlet is not commonly used, and want to
	 * avoid loading all deps. ( for example: jsp servlet )
	 *
	...
	/**
	 * Add a user for the in-memory realm. All created apps use this by default, can
	 * be replaced using setRealm().
	 * 
	 ...
	 *
	 * <p>
	 * Alternatively, you can construct a Connector and set any params, then call
	...
	/**
	 * Sets the current host - all future webapps will be added to this host. When
	 * tomcat starts, the host will be the default host.
	 *
	 ...
	 * @param contextPath The context mapping to use, "" for root context.
	 * @param dir         Base directory for the context, for static files. Must
	 *                    exist, relative to the server home
	 * @return the deployed context
	 ...
	 * @see #addWebapp(String, String)
	 *
	 * @deprecated Use {@link #addWebapp(Host, String, String, LifecycleListener)}
	 *             instead
	 */
	@Deprecated
	...
	/**
	 * @return a pathname to pass to {@link ContextConfig#setDefaultWebXml(String)}
	 *         when using {@link #getDefaultWebXmlListener()}.
	 */
	...
	/**
	 * Create an in-memory realm. You can replace it for contexts with a real one.
	 * The Realm created here will be added to the Engine by default and may be
	 * replaced at the Engine level or over-ridden (as per normal Tomcat behaviour)
	...
	protected void initBaseDir() {
		...
		if (baseFile.exists()) {
			...
			/*
			 * If file permissions were going to be set on the newly created directory, this
			 * is the place to do it. However, even simple calls such as
			 * File.setReadable(boolean,boolean) behaves differently on different platforms.
			 * Therefore, setBaseDir documents that the user needs to do this.
			 */
		}
		try {
	 ...
	 * 
	 * @param silent <code>true</code> sets the log level to WARN for the loggers
	 *               that log information on Tomcat start up. This prevents the
	 *               usual startup information being logged. <code>false</code> sets
	 *               the log level to the default level of INFO.
	 */
	...
	/**
	 * Create the configured {@link Context} for the given <code>host</code>. The
	 * default constructor of the class that was configured with
	 ...
	 *
	 * <p>
	 * The start() method in context will set 'configured' to false - and expects a
	 * listener to set it back to true.
	 */
	...
	/**
	 * Fix reload - required if reloading and using programmatic configuration. When
	 * a context is reloaded, any programmatic configuration is lost. This listener
	 * sets the equivalent of conf/web.xml when the context starts.
	 */
	...
	/**
	 * Helper class for wrapping existing servlets. This disables servlet lifecycle
	 * and normal reloading, but also reduces overhead and provide more direct
	 * control over the servlet.
	 */
	...
	/**
	 * TODO: would a properties resource be better ? Or just parsing /etc/mime.types
	 * ? This is needed because we don't use the default web.xml, where this is
	 * encoded.
	 */
	private static final String[] DEFAULT_MIME_MAPPINGS = { "abs", "audio/x-mpeg", "ai", "application/postscript",
			"aif", "audio/x-aiff", "aifc", "audio/x-aiff", "aiff", "audio/x-aiff", "aim", "application/x-aim", "art",
			"image/x-jg", "asf", "video/x-ms-asf", "asx", "video/x-ms-asf", "au", "audio/basic", "avi",
			"video/x-msvideo", "avx", "video/x-rad-screenplay", "bcpio", "application/x-bcpio", "bin",
			"application/octet-stream", "bmp", "image/bmp", "body", "text/html", "cdf", "application/x-cdf", "cer",
			"application/pkix-cert", "class", "application/java", "cpio", "application/x-cpio", "csh",
			"application/x-csh", "css", "text/css", "dib", "image/bmp", "doc", "application/msword", "dtd",
			"application/xml-dtd", "dv", "video/x-dv", "dvi", "application/x-dvi", "eps", "application/postscript",
			"etx", "text/x-setext", "exe", "application/octet-stream", "gif", "image/gif", "gtar", "application/x-gtar",
			"gz", "application/x-gzip", "hdf", "application/x-hdf", "hqx", "application/mac-binhex40", "htc",
			"text/x-component", "htm", "text/html", "html", "text/html", "ief", "image/ief", "jad",
			"text/vnd.sun.j2me.app-descriptor", "jar", "application/java-archive", "java", "text/x-java-source", "jnlp",
			"application/x-java-jnlp-file", "jpe", "image/jpeg", "jpeg", "image/jpeg", "jpg", "image/jpeg", "js",
			"application/javascript", "jsf", "text/plain", "jspf", "text/plain", "kar", "audio/midi", "latex",
			"application/x-latex", "m3u", "audio/x-mpegurl", "mac", "image/x-macpaint", "man", "text/troff", "mathml",
			"application/mathml+xml", "me", "text/troff", "mid", "audio/midi", "midi", "audio/midi", "mif",
			"application/x-mif", "mov", "video/quicktime", "movie", "video/x-sgi-movie", "mp1", "audio/mpeg", "mp2",
			"audio/mpeg", "mp3", "audio/mpeg", "mp4", "video/mp4", "mpa", "audio/mpeg", "mpe", "video/mpeg", "mpeg",
			"video/mpeg", "mpega", "audio/x-mpeg", "mpg", "video/mpeg", "mpv2", "video/mpeg2", "nc",
			"application/x-netcdf", "oda", "application/oda", "odb", "application/vnd.oasis.opendocument.database",
			...
			"odm", "application/vnd.oasis.opendocument.text-master", "odp",
			"application/vnd.oasis.opendocument.presentation", "ods", "application/vnd.oasis.opendocument.spreadsheet",
			"odt", "application/vnd.oasis.opendocument.text", "otg",
			...
			"application/vnd.oasis.opendocument.text-template", "ogx", "application/ogg", "ogv", "video/ogg", "oga",
			"audio/ogg", "ogg", "audio/ogg", "spx", "audio/ogg", "flac", "audio/flac", "anx", "application/annodex",
			"axa", "audio/annodex", "axv", "video/annodex", "xspf", "application/xspf+xml", "pbm",
			"image/x-portable-bitmap", "pct", "image/pict", "pdf", "application/pdf", "pgm", "image/x-portable-graymap",
			"pic", "image/pict", "pict", "image/pict", "pls", "audio/x-scpls", "png", "image/png", "pnm",
			"image/x-portable-anymap", "pnt", "image/x-macpaint", "ppm", "image/x-portable-pixmap", "ppt",
			"application/vnd.ms-powerpoint", "pps", "application/vnd.ms-powerpoint", "ps", "application/postscript",
			"psd", "image/vnd.adobe.photoshop", "qt", "video/quicktime", "qti", "image/x-quicktime", "qtif",
			"image/x-quicktime", "ras", "image/x-cmu-raster", "rdf", "application/rdf+xml", "rgb", "image/x-rgb", "rm",
			"application/vnd.rn-realmedia", "roff", "text/troff", "rtf", "application/rtf", "rtx", "text/richtext",
			...
			"application/x-sv4cpio", "sv4crc", "application/x-sv4crc", "svg", "image/svg+xml", "svgz", "image/svg+xml",
			"swf", "application/x-shockwave-flash", "t", "text/troff", "tar", "application/x-tar", "tcl",
			"application/x-tcl", "tex", "application/x-tex", "texi", "application/x-texinfo", "texinfo",
			"application/x-texinfo", "tif", "image/tiff", "tiff", "image/tiff", "tr", "text/troff", "tsv",
			"text/tab-separated-values", "txt", "text/plain", "ulw", "audio/basic", "ustar", "application/x-ustar",
			"vxml", "application/voicexml+xml", "xbm", "image/x-xbitmap", "xht", "application/xhtml+xml", "xhtml",
			"application/xhtml+xml", "xls", "application/vnd.ms-excel", "xml", "application/xml", "xpm",
			"image/x-xpixmap", "xsl", "application/xml", "xslt", "application/xslt+xml", "xul",
			"application/vnd.mozilla.xul+xml", "xwd", "image/x-xwindowdump", "vsd", "application/vnd.visio", "wav",
			"audio/x-wav", "wbmp", "image/vnd.wap.wbmp", "wml", "text/vnd.wap.wml", "wmlc", "application/vnd.wap.wmlc",
			"wmls", "text/vnd.wap.wmlsc", "wmlscriptc", "application/vnd.wap.wmlscriptc", "wmv", "video/x-ms-wmv",
			"wrl", "model/vrml", "wspolicy", "application/wspolicy+xml", "Z", "application/x-compress", "z",
	...
	public static void main(String[] args) throws Exception {
		...
		tomcat.addServlet(contextPath, "apiServlet", new GenericServlet() {
			...
			private static final long serialVersionUID = 1360593732387455979L;
